...
Unit tests for blink‐extraction helper functions in pyblinkers:
 ...
 - get_half_height : under the function fit()
...
2. get_half_height:
   ...
   - Uses max_pos_vel_frame/get_left_base/get_right_base internally to
     derive left_base and right_base indices from blink velocity.
   - Returns four floats: (left_zero_half, right_zero_half, left_base_half, right_base_half).
...
def test_left_right_zero_crossing(candidate_signal: np.ndarray, test_df: pd.DataFrame):
    ...
    assert left_z == row["left_zero"], f"left_zero mismatch: got {left_z}, expected {row['left_zero']}"
    assert right_z == row["right_zero"], f"right_zero mismatch: got {right_z}, expected {row['right_zero']}"


def test_get_half_height_all(candidate_signal: np.ndarray, test_df: pd.DataFrame):
    """
    Validate get_half_height across all 5 segments.
    ...
    for idx, row in test_df.iterrows():
        lzh, rzh, lbh, rbh = get_half_height(
            candidate_signal,
            int(row["max_blink"]),
            int(row["left_zero"]),
            int(row["right_zero"]),
